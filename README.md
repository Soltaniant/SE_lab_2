...
<table dir='rtl'>
  <tbody>
    <tr>
      ...
      <td width="454"><p>افزودن کلاس‌های جداگانه برای سرویس‌های پیام‌رسانی (EmailMessageService، SMSMessageService، TelegramMessageService) به منظور رعایت مسئولیت واحد هر کلاس.
استفاده از متود‌های مجزا برای اعتبار‌سنجی شماره‌تلفن، ایمیل و غیره.</p></td>
    </tr>
    <tr>
      <td><p><strong>موارد نقض</strong></p></td>
      <td><p>واسط MessageService و نیز کلاس‌های پیاده‌ساز آن دلایل متعددی برای تغییر دارند. همچنین کلاس Main نقش‌های مختلفی را بازی می‌کند.
      </p></td>
    </tr>
    <tr>
      ...
      <td>
<p>انواع کلاس‌های پیام با توجه به اینکه فرزند Message هستند می‌توانستند به پروژه اضافه شوند. کلاس Main نیز از الگوی Factory برای ایجاد سرویس‌های مختلف استفاده کرده است که به گستزش بدون تغییر کد کمک می‌کند. (در آینده ما نیز از ایده‌ای مشابه همین مسئله استفاده خواهیم کرد)</p>
    </tr>
    <tr>
      <td><p><strong>موارد نقض</strong></p></td>
    ...
    <tr>
      ...
      <td><p><strong>موارد تحقق</strong></p></td>
      <td>
<p>رابطه بین پیام  (کلاس Message) و دیگر انواع پیام به همین گونه است (رابطه is-a) همچنین تا حدودی رابطه مشابهی بین سرویس پیام‌رسانی MessageService و پیاده‌سازی‌های آن وجود دارد، اما دارای مشکلاتی است که رابطه is-a را مخدوش کرده است. </p>
    </tr>
    <tr>
      <td><p><strong>موارد نقض</strong></p></td>
      <td><p>واسط MessageService، رفتارهای اضافی به فرزندان خود تحمیل کرده در حالیکه آن‌ها به این رفتار‌ها نیازی ندارند.</p></td>
    </tr>
    <tr>
      <td rowspan="2"><p>اصل 4</p><p>Interface Segregation Principle</p></td>
      <td><p><strong>موارد تحقق</strong></p></td>
      <td>
<p>واسط MessageService از متود‌های مجزا برای هر یک از سرویس‌ها استفاده کرد است.</p>
    </tr>
    <tr>
      <td><p><strong>موارد نقض</strong></p></td>
      <td><p>واسط MessageService شامل متدهایی است که همه سرویس‌ها به آن نیاز ندارند. لذا بنده آن‌ها خالی رها شده است.</p></td>
    </tr>
    <tr>
      <td rowspan="2"><p>اصل 5</p><p>Dependency Inversion Principle</p></td>
      <td><p><strong>موارد تحقق</strong></p></td>
      <td><p>کلاس Main: وابستگی‌های High-level module به واسط MessageService بجای کلاس‌های خاص (EmailMessageService، SMSMessageService) باعث می‌شود که وابستگی‌ها به صورت معکوس و بر اساس Abstraction باشند. همینطور در مورد Message این مسئله تا حدی رعایت شده است.</p></td>
    </tr>
    <tr>
      <td><p><strong>موارد نقض</strong></p></td>
      <td><p>کلاس Main به طور مستقیم به کلاس‌های خاص سرویس پیام‌رسانی وابسته شده است. می‌توانستیم از تزریق وابستگی یا الگوهایی مانند Factory استفاده کنیم تا وابستگی‌ها به واسط‌ها باشند.</p></td>
    </tr>
  </tbody>
</table>
...
<td width="284">
<p>یک متود sendMessage به صورت جامع در واسط MessageService تعریف شود که وظیفه ارسال پیام (Message) را داراست. سپس هر یک از فرزندان سرویس این متود را به دلخواه خودشان پیاده‌سازی کنند. توجه: برای پیاده‌سازی این سرویس جامع از جنریک‌ها نیز می‌توانستیم بهره ببریم اما صرفا به جهت سادگی بیشتر و نیز پرهیز از overengineering کردن به سراغ آن نرفتیم.</p>
</td>
</tr>
<tr>
<td width="168">
...
<td width="246">
<p>به ازای اضافه کردن یک  نوع جدید از پیام لازم است MessageService و نیز سوییچ‌های Main بروزرسانی شوند. در نتیجه‌ی تغییر واسط MessageService تمامی دیگر پیاده‌سازی‌های آن نیز باید بروز شوند و متود جدید را با بدنه خالی پیاده کنند.</p>
</td>
<td width="284">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        <p>تنها با اضافه کردن تغییرات زیر، این سرویس جدید اضافه می‌شد:
          <ul>
            <li> ساخت پیام جدید</li>
            <li>ساخت سرویس پیام‌رسانی جدید</li>
            <li>افزودن MessageType</li>
            <li>تغییر MessageFacoryImpl</li>
            <li> تغییرات متون ui</li>
          </ul>
